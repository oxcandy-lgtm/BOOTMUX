...
limactl shell "$INSTANCE" -- sh -lc 'set -eu; mkdir -p "$HOME/bin" "$HOME/bootmux"; cp /tmp/bootmux-source/vm/install-codex.sh "$HOME/bootmux/"; cp /tmp/bootmux-source/vm/run-codex-prompt.sh "$HOME/bootmux/"; chmod +x "$HOME/bootmux"/*.sh; cd /tmp/bootmux-source/companion; go test ./...; go test -race ./...; go vet ./...; go build -trimpath -o "$HOME/bin/bootmux-companion" .'
